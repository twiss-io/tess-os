...
# ---------------------------------------------------------------------------
# Doc-test: docs/GATE_QUICKSTART.md's fenced bash block, run VERBATIM
# ---------------------------------------------------------------------------

QUICKSTART_PATH = REPO_ROOT / "docs" / "GATE_QUICKSTART.md"


def _extract_quickstart_script() -> str:
    import re

    text = QUICKSTART_PATH.read_text(encoding="utf-8")
    m = re.search(r"```bash\n(.*?)\n```", text, re.S)
    assert m, "docs/GATE_QUICKSTART.md: no fenced ```bash block found"
    return m.group(1)


@pytest.fixture
def fresh_framework_checkout(tmp_path):
    """A fresh 'clone' fixture: exactly what `git clone` of this framework
    gives you BEFORE running `tessctl init` — the pristine `.tess/core`
    mirror, the committed `tess.lock` + `tess.manifest.json`, and the
    engine/wrapper — nothing else. `tessctl init` (the quickstart's own
    first command) is what materializes the rest of the live tree, exactly
    as it would for a real adopter."""
    root = tmp_path / "quickstart-repo"
    root.mkdir()
    shutil.copytree(REPO_ROOT / ".tess" / "core", root / ".tess" / "core")
    (root / ".tess" / "bin").mkdir(parents=True, exist_ok=True)
    shutil.copy2(REPO_ROOT / ".tess" / "bin" / "tessctl", root / ".tess" / "bin" / "tessctl")
    os.chmod(root / ".tess" / "bin" / "tessctl", 0o755)
    shutil.copy2(REPO_ROOT / ".tess" / "tess.lock", root / ".tess" / "tess.lock")
    shutil.copy2(REPO_ROOT / "tess.manifest.json", root / "tess.manifest.json")
    shutil.copy2(REPO_ROOT / "tessctl", root / "tessctl")
    os.chmod(root / "tessctl", 0o755)

    _init_repo(root)
    _git(root, "add", "-A")
    _git(root, "commit", "-q", "-m", "baseline: framework core only, pre tessctl-init")
    return root


def test_gate_quickstart_doc_runs_verbatim_end_to_end(fresh_framework_checkout, tmp_path):
    """Extracts docs/GATE_QUICKSTART.md's ENTIRE fenced bash block and runs
    it, unmodified, against a fresh framework checkout. The script itself
    is self-verifying (it asserts the BLOCKED step failed and exits
    non-zero if it unexpectedly didn't), so a passing run here proves the
    documented walkthrough is truthful, command for command — not just that
    a hand-written test mirrors what the doc claims."""
    script = _extract_quickstart_script()
    remote = tmp_path / "quickstart-remote.git"
    # The doc's own script path (/tmp/gate-quickstart-remote.git) is fine to
    # run as-is in CI too; give it a unique per-test remote via env to avoid
    # any cross-test collision if tests ever run in parallel on one host.
    script = script.replace("/tmp/gate-quickstart-remote.git", str(remote))

    r = subprocess.run(
        ["bash", "-c", script],
        cwd=str(fresh_framework_checkout),
        capture_output=True, text=True,
    )
    combined = r.stdout + r.stderr
    assert r.returncode == 0, combined
    assert "doctor: OK" in combined
    assert "As expected: BLOCKED" in combined
    assert "CLEARED — the signed, covering verdict satisfied the gate." in combined
