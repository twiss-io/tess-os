...
def test_return_manifest_artifact_path_rejects_external_escape_symlink_and_nonregular_file(engine, tmp_path):
    """The shared lint boundary rejects every unsafe artifact path shape."""
    root = tmp_path / "os"
...
def test_return_manifest_lint_without_root_fails_closed(engine, tmp_path):
    ...
    manifest = _valid_return_manifest()
    manifest["artifacts"] = [{"path": str(artifact), "description": "external"}]
...
def test_cli_validate_return_manifest_accepts_contained_regular_file_and_rejects_escape_paths(
    ...
    """CLI validation uses the same evidence containment primitive as run."""
