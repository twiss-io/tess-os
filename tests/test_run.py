...
def test_run_artifact_hash_keeps_opened_file_during_directory_swap(engine, tmp_path, monkeypatch):
    ...
    This models the check-then-swap window Cyra found: the final file name is
    about to be opened after the trusted parent directory has already been
    inspected.  The attacker replaces that directory name with a symlink to
    external content.  Binding must still hash the file obtained from the
    already-opened in-root directory descriptor, never the new external
    pathname.
    """
    root = tmp_path / "os"
