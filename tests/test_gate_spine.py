...
def _policy_with_verifier_keys(root, keys):
    ...
    policy["policy"]["signoff_keys"] = signoff_keys
    return policy
...
def gate_repo(project, verifier_gpg_keys):
    ...
    actually verify; honesty-capstone-audit-2026-07-08 §3-d: also
    registering a signoff_keys entry so a real, signed hard-floor sign-off
    can actually clear), one initial commit."""
    root = project.root
...
def test_ci_blocks_hard_floor_match_even_with_covering_signed_approve_verdict(
    ...
    _write_verdict(
        ...
        _valid_verdict(
            covers_paths=["payments/**"],
            artifact_hashes={"payments/charge.py": _blob_sha(gate_repo, "payments/charge.py")},
            engine=engine,
            keys=verifier_gpg_keys,
        ),
    )
...
def test_ci_blocks_hard_floor_match_with_unsigned_forged_signoff_artifact(gate_repo, run_cli):
    ...
    pre-fix) must NOT clear a hard floor. This is the non-vacuous 'fails on
    unpatched, passes with the fix' proof for (d): against the unpatched
...
def test_ci_blocks_hard_floor_match_with_hand_faked_signature_signoff(gate_repo, run_cli):
    ...
    signoff["signature"] = {
        ...
        "signature_armored": (
            "-----BEGIN PGP SIGNATURE-----\n\nSGFuZC1mYWtlZCwgbm90IGEgcmVhbCBzaWduYXR1cmUu\n"
            "-----END PGP SIGNATURE-----\n"
        ),
    }
    (signoff_dir / "money.signoff.json").write_text(json.dumps(signoff), encoding="utf-8")
...
def test_ci_fails_closed_when_policy_file_missing(gate_repo, run_cli):
    ...
    payload = json.loads(r.stdout)
    assert payload["blocked"] is True
    assert "POLICY_INVALID: the effective policy is invalid" in payload["reasons"]
...
def test_pre_push_requires_both_base_and_head_or_neither(gate_repo, run_cli):
    ...
    payload = json.loads(r.stdout)
    assert payload["blocked"] is True
    assert "GATE_INPUT_INVALID: gate invocation input is invalid" in payload["reasons"]
...
def test_covering_verdict_only_clears_its_reviewed_content_not_a_later_edit(gate_repo, run_cli, engine, verifier_gpg_keys):
    ...
    assert payload["blocked"] is True
    assert "VERDICT_CONTENT_STALE: a covering verdict is not bound to current content" in payload["reasons"]
...
def test_allowed_verifiers_is_enforced_wrong_domain_verifier_does_not_clear(gate_repo, run_cli, engine, verifier_gpg_keys):
    ...
    payload = json.loads(r.stdout)
    assert payload["blocked"] is True
    assert "VERIFIER_NOT_ALLOWED: no covering verdict is from an allowed verifier" in payload["reasons"]
