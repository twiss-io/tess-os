...
| Coding-agent integration | Claude Code target/driver; Codex target/driver; Gemini CLI target; generic `AGENTS.md` output. | Enforcement differs by runtime: Claude Code Enforced; Codex and Gemini CLI Partial; other `AGENTS.md` tools Advisory. Claude is an uncertified preview; generic output is not native feature parity. |
