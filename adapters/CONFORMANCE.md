...
| Qwen Code | none (reads `AGENTS.md`) | Advisory | Reads `AGENTS.md` alongside `QWEN.md`. | Claude-style `settings.json` hooks exist, but Tess renders no Qwen settings and the similarity is untested. | [memory](https://github.com/QwenLM/qwen-code/blob/main/docs/users/features/memory.md), [hooks](https://github.com/QwenLM/qwen-code/blob/main/docs/users/features/hooks.md) |
