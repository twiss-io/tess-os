...
# (G16 / QW3, audit reform 2026-06-10): timestamps ending in "Z" (tool output,
# logs, message metadata) are UTC, and misreading UTC timestamps as local time
# can mistime scheduled actions.
...
echo "UTC=${utc}Z / LOCAL=${local_time} (${local_tz}, UTC${offset}). Timestamps ending in Z are UTC: convert to your local timezone (TESS_LOCAL_TZ=${local_tz}) before any time-of-day reasoning."
