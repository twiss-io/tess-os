# Contracts — Phase 0 (Contracts-as-Code) + Phase 2 (`policy.schema.json`)

> Spec: `docs/ULTIMATE_FRAMEWORK_PLAN.md` Phase 0 and Design Decision #3 ("Contracts become schemas"); Phase 2 and Design Decisions #2/#6 for `policy.schema.json`.
...
- `tessctl doctor` / `tessctl verify` / `tessctl lock --check` cover these
  seven files like any other core-managed entry: unpinned `.tess/core`
  tamper is flagged as CORE TAMPER (SECURITY-TIER for the three schemas +
  policy.yaml above), and live drift from the pristine core is flagged and
  remediated the same way as any other doctrine file.
