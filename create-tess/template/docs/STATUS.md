...
| Codex target and driver | **C2 — manual-gated compatibility** | The renderer emits `AGENTS.md`, `.codex/config.toml`, prompt files, and one `.codex/agents/<name>.toml` per installed role (sandbox mode from the role file). The driver is not live-tested against native event samples and does not have native-parity certification. |
