...
keyword subset `schema/*.schema.json` in this component use: type,
required, properties, additionalProperties, enum, items. This mirrors this
repo's own documented convention (every `core/contracts/*.schema.json` file
ends with a "Minimal-validator keyword subset supported by tessctl
validate" `$comment`) AND intent_router.schema_check's identical
convention, without importing or depending on either `.tess/bin/tessctl`'s
internal validator or the sibling `intent_router` package — keeping this
component importable and testable entirely on its own.

This is a verbatim-in-spirit duplicate of intent_router/intent_router/
schema_check.py, not a shared import: each top-level tess-os component
stays independently deployable with zero cross-component import edges
(see spec-engine/README.md "Why this duplicates intent_router.schema_check
