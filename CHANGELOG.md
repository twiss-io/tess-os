...
**Known upgrade issue (v0.2.0):** `tessctl update` from a pre-0.2.0 install can leave `.tess/core/roster-paths.json` and `.tess/core/templates/claude-md/hard-floor.md` untracked (doctor/verify FAIL: UNTRACKED CORE FILE) when that install already has a same-path file from an older feature (the curated-squad config) whose content differs from what 0.2.0 ships. Nothing is lost or overwritten -- the adopt step (A2) deliberately never clobbers an existing file -- but the two files stay unprotected until fixed by hand: `tessctl lock --regen --only .tess/core/roster-paths.json --only .tess/core/templates/claude-md/hard-floor.md --yes`. Confirmed via the OTA rehearsal kit (0.1.4 -> 0.2.0): every other check passes, including the two negative/security cases (wrong signing key, non-annotated tag) and the AGENTS.md hand-edit path (correctly re-rendered on update, with the hand edit preserved in a pre-image snapshot).
