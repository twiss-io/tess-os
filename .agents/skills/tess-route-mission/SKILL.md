...
<!-- Rendered by `tessctl render --target codex` from .tess/core/commands/route-mission.md. Regenerate; do not hand-edit. -->

Tess OS command `/route-mission`, packaged as an Agent Skill. Run it only when the user asks for it by name (`$tess-route-mission` in Codex).
